...
class ExecutionContextManager:
    def __init__(self, plan_graph: dict, session_id: str = None, original_query: str = None, file_manifest: list = None, debug_mode: bool = False):
        ...
        for edge in plan_graph.get("edges", []):
            # ONLY accept standardized format as defined in planner prompt
            if "source" in edge and "target" in edge:
                self.plan_graph.add_edge(edge["source"], edge["target"])
            else:
                raise ValueError(f"Invalid edge format: {edge}. Expected format: {{'source': 'node_id', 'target': 'node_id'}}. Check PlannerAgent output.")
        ...
        if writes:
            for write_key in writes:
                ...
                if execution_result and execution_result.get("status") == "success":
                    # Fix: Data is in code_results.result, not result directly
                    code_results = execution_result.get("code_results", {})
                    if code_results.get("status") == "success":
                        result_data = code_results.get("result", {})
                    else:
                        result_data = execution_result.get("result", {})
